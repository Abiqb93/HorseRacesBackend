...
app.get('/api/performance_predictions/horse', (req, res) => {
  // Accept horseName as provided, but normalize spacing
  const horseName = (req.query.horseName || "").trim();
  ...
  db.query(sql, [horseName], (err, results) => {
    if (err) {
      console.error("❌ Error fetching performance_predictions:", err);
      return res.status(500).json({ error: "Database error" });
    }

    // Return the full set of rows (even if it's just 1)
    return res.status(200).json({ data: results });
  });
});
...
app.post("/api/horseTracking", (req, res) => {
  ...
  if (!horseName || !trackingDate || !finalUser || !finalTrackingType) {
    return res.status(400).json({
      error: "horseName, trackingDate, user, and trackingType are required.",
    });
  }
...
// ✅ PATCH: Rename/change one tracking category
app.patch("/api/horseTracking/:horseName/type", (req, res) => {
  ...
  const finalOldType = oldTrackingType || trackingType || TrackingType || null;
  const finalNewType = newTrackingType || null;

  if (!user) {
    return res.status(400).json({ error: "Missing user parameter" });
  }

  if (!finalOldType || !finalNewType) {
    return res.status(400).json({
      error: "oldTrackingType and newTrackingType are required.",
    });
  }
  ...
  db.query(
    duplicateCheckQuery,
    [horseName, user, finalNewType],
    (dupErr, duplicateRows) => {
      if (dupErr) {
        console.error("Error checking duplicate TrackingType:", dupErr);
        return res.status(500).json({ error: "Database error" });
      }

      if (duplicateRows.length > 0) {
        return res.status(409).json({
          error: "Horse is already tracked under the new category.",
        });
      }
      ...
      db.query(
        updateQuery,
        [finalNewType, horseName, user, finalOldType],
        (err, result) => {
          if (err) {
          ...
          return res.status(200).json({
            message: "Tracking category updated.",
            horseName,
            user,
            oldTrackingType: finalOldType,
            newTrackingType: finalNewType,
...
// ============================================================
// DYNAMIC REVIEW CONDITIONS API
// ============================================================
//
// Assumptions:
//
// 1. `app` is your Express app
// 2. `db` is your existing MySQL connection
// 3. review_conditions table already exists
//
// review_conditions expected columns:
//
// id
// condition_name
// userId
...
//
// to store whether each user wants:
//
// default_only
...
// ============================================================
// ENSURE review_rule_preferences EXISTS
// ============================================================
...
const ensureReviewRulePreferencesTable = () => {
  ...
  db.query(query, (err) => {
    if (err) {
      console.error(
        "Error creating/checking review_rule_preferences:",
        err
      );
      return;
    }
...
const VALID_RULE_METRICS = new Set([
  "total_runs",
  ...
  "latest_minus_previous_max",
  "latest_minus_previous_min",

  "latest_minus_previous_avg",
  "latest_minus_previous_mean",
...
const validateRuleNode = (
  ...
  if (
    ...
    return "Each rule must be a JSON object.";
  }


  if (depth > 10) {
    return "Rule nesting is too deep.";
  }
  ...
  if (
    ...
    if (
      !Array.isArray(node.all) ||
      node.all.length === 0
    ) {
      return "'all' must contain at least one rule.";
    }


    for (const child of node.all) {

      const error = validateRuleNode(
        child,
        depth + 1
      );
  ...
  if (
    ...
    if (
      ...
      return "'any' must contain at least one rule.";
    }
  ...
  if (
    !VALID_RULE_METRICS.has(
      metric
    )
  ) {
    return `Unsupported metric: ${metric || "(blank)"}`;
  }
  ...
  if (
    ...
    if (!field) {
      return `Metric '${metric}' requires a field.`;
    }


    // Only permit normal dataframe-style
    // column names.
    if (
  ...
  const operatorNotRequired =
    metric === "latest_contains" ||
    metric === "latest_not_contains" ||
    metric === "latest_in" ||
    metric === "latest_not_in";


  if (
    !operatorNotRequired &&
    metric !== "count_previous"
  ) {
    ...
    if (
      !VALID_RULE_OPERATORS.has(
        operator
      )
    ) {
      return `Unsupported operator: ${operator || "(blank)"}`;
    }
  }
  ...
  if (
    ...
    return `Metric '${metric}' requires a value.`;
  }


  // ==========================================================
  // COUNT PREVIOUS SPECIAL CASE
  // ==========================================================
  ...
  if (
    ...
    const countOperator = String(
      node.operator || ""
    ).trim();


    const fieldOperator = String(
      node.field_operator || ""
    ).trim();
    ...
    if (
      ...
      return "count_previous requires a valid operator.";
    }
    ...
    if (
      ...
      return "count_previous requires a valid field_operator.";
    }
    ...
    if (
      ...
      return "count_previous requires field_value.";
    }
  }
...
// - conditions created by this particular user
//
// Does NOT expose other users' rules.
// ============================================================
    ...
    db.query(
      ...
      (err, results) => {
        ...
        if (err) {
          ...
          console.error(
            "Error fetching review condition:",
            err
          );
...
//
// Creates USER condition only.
//
// Frontend users are NOT allowed to create
// is_default = 1 conditions.
// ============================================================
  ...
  (req, res) => {
    ...
    if (!ruleJson) {
      ...
      return res
        ...
        .json({
          error:
            "rule_json must be a valid JSON object",
        });
    }
    ...
    db.query(
      ...
      (
        ...
        if (
          ...
          console.error(
            "Error checking duplicate review rule:",
            duplicateErr
          );
...
// is_default = 0
//
// Default rules therefore cannot be changed.
// ============================================================
    ...
    db.query(
      ...
      (
        ...
        if (
          ...
          console.error(
            "Error checking review-rule ownership:",
            ownershipErr
          );
        ...
        if (
          ...
          return res
            ...
            .json({
              error:
                "Rule not found or you are not allowed to edit it",
            });
        }
            ...
            db.query(
              ...
              (
                ...
                if (
                  ...
                  console.error(
                    "Error updating review condition:",
                    updateErr
                  );
    ...
    db.query(
      ...
      (
        ...
        if (err) {
          ...
          console.error(
            "Error deleting review condition:",
            err
          );
        ...
        if (
          ...
          return res
            ...
            .json({
              error:
                "Rule not found or you are not allowed to delete it",
            });
        }
    ...
    db.query(
      ...
      (
        ...
        if (err) {
          ...
          console.error(
            "Error fetching review rule preferences:",
            err
          );
        ...
        // User never saved a preference.
        // Default to default + defined.
        if (
          !results ||
          results.length === 0
        ) {
    ...
    db.query(
      ...
      (err) => {
        ...
        if (err) {
          ...
          console.error(
            "Error saving review rule preference:",
            err
          );
...
app.patch('/api/APIData_Table2/race/tags', (req, res) => {
  ...
  const sql = `
    UPDATE APIData_Table2
    SET tagging = JSON_ARRAY_APPEND(
      COALESCE(NULLIF(tagging, ''), JSON_ARRAY()),
      '$',
      CAST(? AS JSON)
    )
    WHERE raceTitle = ?
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result.affectedRows === 0) {
        return res.status(404).json({ error: 'Race records not found' });
      }
