...
//
// Where the connection comes from lives in ./dbConfig.cjs, shared with the
// France ingestion so the two cannot disagree about which variables count.
const { resolveDbConfig, missingCredentials } = require("./dbConfig.cjs");

const dbConfig = resolveDbConfig();
const missingDbVars = missingCredentials(dbConfig);

// A missing credential is loud but not fatal.
//
...
if (missingDbVars.length) {
  console.error(
    ...
    "Set DB_HOST, DB_USER and DB_PASSWORD on the Railway service (or supply\n" +
    "DATABASE_URL). Database-backed routes will fail until then.\n" +
